 ...
 */
 ...
 export class AddVehiclePage {
 	...
 	constructor(public navCtrl: NavController, public navParams: NavParams, public alertCtrl: AlertController, public storage: Storage, public toastCtrl: ToastController) {
 		this.storage.get('car').then(data => {
 	...
 	create(){  
 		...
 		if (this.check()){
 			...
 			this.vehicles.push({brand:this.brand, model:this.model, capacity:this.capacity, colour: this.colour, license:this.license});
 			this.storage.set('car', this.vehicles).then(data => this.toasting(data)).catch(err => this.toasting(err));
 			console.log('Worked');
